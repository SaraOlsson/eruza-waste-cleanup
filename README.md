...
- People could register in the app before dropping their bottles into an IoT-connected waste bin. Thus, Once they drop the bottles, the number of bottles is auto-calculated and the count is converted into the appropriate reedem points that can be attached to the bar code that is tagged with their user account. They can encash their points to buy food from any partnered food stalls all over the country.
